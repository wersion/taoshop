...
<!--
<?php foreach (yii::t('shipping', 'js_languages') as $key=>$item):?>
var <?=$key?> = "<?=$item?>";
<?php endforeach;?>
-->
</script>
...
<body>
  ...
  <form action="<?=Url::toRoute('/shipping/do-edit-print-template') ?>" enctype="multipart/form-data" method="post" name="theForm" id="theForm" onsubmit="return validate();">
  ...
  <input type="hidden" name="shipping_name" value="<?=$shipping['shipping_name']?>">
    ...
    <tr>
      <td style="overflow:hidden; background-color:#EFEFDE; padding-left:8px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0" height="50">
          <tr>
            <td align="left">
                <?=Html::dropDownList('lable','select_notice',Yii::t('shipping', 'lable_box'),['id'=>'lable','class'=>'select_box','onchange'=>"javascript:call_flash('lable_add',this);"])?>
              <input type="button" name="del" id="del" value="<?=Yii::t('shipping','del_lable')?>" onclick="javascript:call_flash('lable_del', this);">
            </td>
            <td id="pic_control_upload" <?php if ($shipping['print_bg'] != ''):?>class="display_no"<?php endif;?>>
              <input type="file" name="bg" id="bg" <?php if ($shipping['print_bg'] != ''):?> disabled="disabled" <?php endif;?>>
              <input type="button" name="upload" id="upload" value="<?=Yii::t('shipping', 'upload_shipping_bg')?>" onclick="javascript:bg_upload();" <?php if ($shipping['print_bg'] != ''):?> disabled="disabled"<?php endif;?>>
              <iframe id="bg_upload_hidden" name="bg_upload_hidden" frameborder="0" scrolling="no" class="display_no"></iframe>
            </td>
            <td id="pic_control_del" <?php if ($shipping['print_bg'] == ''):?> class="display_no" <?php endif;?>>
              <input type="button" name="upload_del" id="upload_del" onclick="javascript:bg_del();" value="<?=Yii::t('shipping', 'del_shipping_bg')?>" <?php if ($shipping['print_bg'] == ''):?> disabled="disabled" <?php endif;?>>
            </td>
            <td align="right"><input type="button" value="<?=Yii::t('shipping', 'save_setting')?>" onclick="javascript:save();">&nbsp;&nbsp;<input type="button" value="<?=yii::t('shipping', 'recovery_default')?>" onclick="javascript:recovery_default();"></td>
          </tr>
          <tr>
            <td colspan="3">&nbsp;</td>
          </tr>
    ...
    <tr>
      ...
      <param name="movie" value="/images/pint.swf">
...
function bg_upload()
{
  ...
  //he_form.act.value = 'print_upload';
  the_form.action = '<?=Url::to('/shipping/print-upload') ?>';
  the_form.submit();
