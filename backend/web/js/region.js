...
region.loadRegions = function(parent, type, target)
{
  //Ajax.call(region.getFileName(), 'type=' + type + '&target=' + target + "&parent=" + parent , region.response, "GET", "JSON");
	$.ajax({
		...
		data:'dep='+type+'&area_code='+parent+'&target='+target,
...
region.loadProvinces = function(country, selName)
{
  ...
  region.loadRegions(country, 2, objName);
}
...
region.response = function(result, text_result)
{
  ...
  if (result.regions)
  {
    for (i = 0; i < result.regions.length; i ++ )
    {
      ...
      opt.value = result.regions[i].area_code;
      opt.text  = result.regions[i].area_name;
